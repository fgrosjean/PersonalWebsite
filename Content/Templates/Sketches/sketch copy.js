...
function draw() {
	...
	for (let i = 1; i <= gridSize; i++) {
		for (let j = 1; j <= gridSize; j++) {
			...
			squareModule(posX, posY, gridModuleSize, -((i * j) * 0.1), i * j)
		}
	}
}
...
function squareModule(x, y, squareSize, delay, index) {
	...
	const percentageCompleted = map(currentTime, 0, 1, 1, 100).toFixed(0)
	
	switch (true) {
		case percentageCompleted > 80: break
		...
		for (let i = 1; i <= 3; i++) {
			...
			rectMode(CORNERS)
			fill(255, 0.33)
			const sizeOutro = map(percentageCompleted, 60, 90 - (10 * 1) - 1, 0, squareSize, true)
			rect(x + sizeOutro, y + sizeOutro, x + squareSize , y + squareSize, borderRadius)
			rectMode(CORNER)
			blendMode(BLEND)
		}
		
		if (index == 1 &&  percentageCompleted == 65) {
		...
		for (let i = 1; i <= 3; i++) {
			blendMode(ADD)
			fill(255, 0.33)
			const sizeIntro = mapWithEase(percentageCompleted, 1, 30 + (10 * i), 1, squareSize, true, false)
